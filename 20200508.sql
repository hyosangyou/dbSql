...
NO NULL 제약조건
: 컬럼에 값이 NULL이 들어오는 것을 방지하는 제약조건
...
INSERT INTO dept_test VALUES(99, 'ddit', 'daejeon');
commit;
-- deptno 컬럼은 dept_test테이블의 deptno 컬럼을 참조한다.;
...
INSERT INTO emp_test VALUES(9999, 'brow', 99);

98번 부서는 존재 하지 않으므로 정상적으로 입력할 수 없다.
INSERT INTO emp_test VALUES(9999, 'brow', 98);

PRIMARY KEY 제약조건 테이블 레벨에서 기술
...
부모 레코드(dept_test.deptno)를 참조하고 있는 자식 레코드(emp_test.eptno)가
...
2. 테이블 명세서가 정확하지 않으면 신규 개발자는 해당 내용을 모를 수가 있음
 ...
 신기하게도 emp 테이블의 데이터가 삭제되거나 null로 설정되는 경우를 볼 수 있음
...
dept_test 테이블 복사 =-> dept_test_copy
...
이미 생성된 테이블을 수정 할 수도 있음
   1. 새로운 컬럼을 추가
     ********** 새로운 컬럼은 테이블의 마지막 컬럼으로 추가 된다.
     ********** 기존 생성되어 있는 컬럼 중간에는 새로운 컬럼을 추가할 수 가 없다.
   ...
   2. 기존 컬럼 삭제, 컬럼 이름 변경, 컬럼 사이즈 변경, (타입도 제한적으로 변경 가능)
      ...
      dept_test_.deptno 이름변경하여 dept_test.dno로 수정하더라도 fk는 변경된 이름으로 잘 유지가 된다.
   3. 제약 조건 추가
...
위에서 추가한 hp 컬럼의 컬럼 사이즈를 varchar(30)으로 변경
ALTER TABLE emp_test MODIFY (hp varchar(30));

○컬럼 타입 변경

위에서 추가한 hp 컬럼의 타입을 VARCHAR(30)에서 DATE로 변경
...
위에서 추가한 emp_test 테이블의 hp 컬럼을 hp_n으로 이름 변경 하여라
